...
import com.olive.framework.annotation.Excel;
import com.olive.framework.annotation.Excel.ColumnType;
import com.olive.framework.annotation.Excel.Type;
import com.olive.framework.annotation.Excels;
import com.olive.framework.cache.CacheService;
import com.olive.framework.config.AppConfig;
import com.olive.framework.config.ExcelHandlerAdapter;
...
import com.olive.framework.util.file.ImageUtils;
import com.olive.framework.util.reflect.ReflectUtils;
import com.olive.framework.web.system.SysDictData;
...
public class ExcelUtil<T> {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (String.class == fieldType) {
                        ...
                        if (val instanceof String) {
                            val = org.apache.commons.lang3.time.DateUtils.parseDate(val.toString(), parsePatterns);
    ...
    public int getImageType(byte[] value) {
        String type = FileUtils.getFileExtendName(value);
    ...
    public static String convertDictByExp(String dictValue, String dictType, String separator) {
        ...
        List<SysDictData> datas = (List<SysDictData>) cacheService.get(CacheConstants.SYS_DICT_KEY + ":" + dictType, List.class);
        // List<SysDictData> datas = (List<SysDictData>) CacheUtils.getCache(CacheConstants.SYS_DICT_KEY).get(dictType, List.class);
    ...
    public static String reverseDictByExp(String dictLabel, String dictType, String separator) {
        ...
        List<SysDictData> datas = (List<SysDictData>) cacheService.get(CacheConstants.SYS_DICT_KEY + ":" + dictType, List.class);
        // List<SysDictData> datas = (List<SysDictData>) CacheUtils.getCache(CacheConstants.SYS_DICT_KEY).get(dictType, List.class);
        if (org.apache.commons.lang3.StringUtils.containsAny(separator, dictLabel) && CollectionUtils.isNotEmpty(datas)) {
            for (SysDictData dict : datas) {
    ...
    public String parseDateToStr(String dateFormat, Object val) {
        ...
        if (val instanceof Date d) {
            str = new SimpleDateFormat(dateFormat).format(d);
        } else if (val instanceof LocalDateTime d) {
            str = LocalDateUtil.dateTimeToStr(d, dateFormat);
            // str = DateUtils.parseDateToStr(dateFormat, DateUtils.toDate((LocalDateTime) val));
        } else if (val instanceof LocalDate d) {
            str = LocalDateUtil.dateToStr(d, dateFormat);
